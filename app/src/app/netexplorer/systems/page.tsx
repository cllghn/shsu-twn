...
const SystemsPageContent: React.FC = () => {
    ...
    const nodeKeys = Object.values(metadata.systems.kvs);
    const menuItems = nodeKeys.sort((a, b) => a.localeCompare(b));

    const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
    const [selectedItem, setSelectedItem] = useState("select a system"); // Default text
    ...
    const filterDataBySource = useCallback((selected: string) => {
        if (!selected || selected === "select a system") return null;
        // Match the selected string value to the key on the metadata.systems.kvs
        const matchedKey = Object.keys(metadata.systems.kvs).find(
            key => metadata.systems.kvs[key] === selected
        );
        ...
        // Go back one degree of separation to get the source of the selected 
        // node's data... To be clear, this could be expanded back until we
        // hit on the actual water source, but this will work for now.
        const waterInputs = graphData.elements.edges
            .filter(edge => edge.data.target === matchedKey);
        // console.log("waterInputs", waterInputs);

        // Go out one degree of separation to get the targets of the selected 
        // node's data.
        const waterOutputs = graphData.elements.edges
            .filter(edge => edge.data.source === matchedKey);
        // console.log("waterOutputs", waterOutputs);

        // Combine unique edge ids into a single array
        const edgeIds = Array
...

                        {
                            filteredData ? (
                                <div className="pt-6 font-bold text-sm">
                                    <InfoIcon /> The graph below shows water flowing into and out from the selected water system. It omits inputs and outputs that are not directly connected to the selected node.
                                </div>) :
                                <div></div>
                        }
                    </Paper>
                            ...
                            <Box sx={{ width: '100%' }}>
                                ...
                                <TabPanel value={activeTab} index={0}>
                                    <DynamicGraph
                                        ...
                                        selected={
                                            Object
                                                .keys(metadata.systems.kvs)
                                                .find(key => metadata.systems.kvs[key] === filteredNode) || ""
                                        }
                                    />
                                </TabPanel>
                                ...
                                <TabPanel value={activeTab} index={1}>
                                    ...
                                    <div className="flex pt-4 justify-center items-center">
                                        <NodeVolumeScoreCards
                                            data={filteredData}
                                            selected={
                                                Object.keys(metadata.systems.kvs).find(
                                                    key => metadata.systems.kvs[key] === filteredNode
                                                ) || ""
                                            } />
                                    </div>
